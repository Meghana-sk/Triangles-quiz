...
const checkAnglesOfTriangle = () => {
  if (
    Number(angles[0].value) > 0 &&
    Number(angles[0].value) <= 360 &&
    Number(angles[1].value) > 0 &&
    Number(angles[1].value) <= 360 &&
    Number(angles[2].value) > 0 &&
    Number(angles[2].value) <= 360
  ) {
    if (
      ...
      result.innerText =
        "No. Angles you have inputted are not angles of triangle";
    }
  } else {
